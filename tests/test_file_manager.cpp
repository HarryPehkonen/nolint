...
public:
    ...
    MOCK_METHOD(void, write_file, (const std::string& path, const std::vector<std::string>& lines),
                (override));
...
protected:
    ...
    void SetUp() override { mock_fs = std::make_unique<MockFileSystem>(); }

    void CreateFileManager() { file_manager = std::make_unique<FileManager>(std::move(mock_fs)); }
};

TEST_F(FileManagerTest, ExtractIndentationWithSpaces) {
    // Arrange
    std::vector<std::string> file_contents = {"#include <iostream>", "", "int main() {",
                                              "    int x = 42;", // 4 spaces
                                              "    return 0;",       "}"};

    EXPECT_CALL(*mock_fs, read_file("/test.cpp")).WillOnce(Return(file_contents));
...
TEST_F(FileManagerTest, ExtractIndentationWithTabs) {
    // Arrange
    std::vector<std::string> file_contents = {"void foo() {",
                                              "\tint x = 42;", // 1 tab
                                              "}"};
...
TEST_F(FileManagerTest, NolintnextlineIndentation) {
    ...
    std::vector<std::string> file_contents
        = {"#include <iostream>", "", "int main() {", "    int x = 42;", "    return 0;", "}"};

    EXPECT_CALL(*mock_fs, read_file("/test.cpp")).WillOnce(Return(file_contents));

    EXPECT_CALL(*mock_fs, write_file("/test.cpp", ::testing::_))
        .WillOnce([](const std::string&, const std::vector<std::string>& lines) {
            // Verify the NOLINTNEXTLINE is properly indented (with line endings applied)
...
TEST_F(FileManagerTest, MixedIndentationLevels) {
    // Arrange
    std::vector<std::string> file_contents = {"void process() {",
                                              "    if (condition) {",
                                              "        for (int i = 0; i < 10; ++i) {",
