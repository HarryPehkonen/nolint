...
auto NolintProcessor::process_warnings(std::istream& input) -> void {
    ...
    for (size_t i = 0; i < warnings.size(); ++i) {
        ...
        try {
            ...
            NolintStyle current_style = simple_ui ? simple_ui->get_current_style() : NolintStyle::NOLINT_SPECIFIC;
            
            // Build context for display
            auto context = context_builder_.build_context(warning, file_lines, current_style);
            context.current = static_cast<int>(i + 1);
            context.total = static_cast<int>(warnings.size());
            ...
            // Handle the action (may need to redisplay for style changes)
            bool should_continue = handle_user_action(action, warning);
            if (!should_continue) {
                break;  // User chose to quit
            }
            
            // If user changed style, redisplay the same warning
            if (action == UserAction::STYLE_UP || action == UserAction::STYLE_DOWN) {
                --i;  // Process the same warning again with new style
            }
            
        } catch (const std::exception& e) {
            std::cerr << "Error processing warning in " << warning.file_path 
                     << ":" << warning.line_number << " - " << e.what() << "\n";
            continue;  // Skip this warning and continue
        }
    }
...
auto NolintProcessor::create_modification(const Warning& warning, NolintStyle style) -> Modification {
    Modification mod;
