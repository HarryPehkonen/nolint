...
auto filter_warnings(const std::vector<Warning>& warnings, const std::string& filter) -> std::vector<size_t> {
    ...
    for (size_t i = 0; i < warnings.size(); ++i) {
        ...
        std::string searchable_text = warning.file_path + " " + warning.type + " " + warning.message;
        std::transform(searchable_text.begin(), searchable_text.end(), searchable_text.begin(), ::tolower);
...
auto calculate_warning_statistics(const std::vector<Warning>& warnings, 
    ...
    for (size_t i = 0; i < warnings.size(); ++i) {
        ...
        NolintStyle style = (decision_it != decisions.end()) ? decision_it->second : NolintStyle::NONE;
    ...
    switch (event) {
        ...
        case InputEvent::ARROW_UP:
            if (model.show_statistics) {
                ...
                model.decisions[model.current_warning_original_index()] = static_cast<NolintStyle>(current);
