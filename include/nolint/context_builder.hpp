...
public:
    ...
    auto build_context(const Warning& warning, const std::vector<std::string>& file_lines,
                       NolintStyle style = NolintStyle::NOLINT_SPECIFIC,
                       int context_size = DEFAULT_CONTEXT_SIZE) -> WarningContext;

private:
    auto extract_context_lines(const std::vector<std::string>& file_lines, int warning_line,
